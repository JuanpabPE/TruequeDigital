...
export const MembershipProvider = ({ children }) => {
    ...
    const createMembership = async (membershipData) => {
        try {
            ...
            const errorMsg = error.response?.data?.message || 'Error al crear membresía';
            setError(errorMsg);
    ...
    const cancelMembership = async () => {
        try {
            ...
            const errorMsg = error.response?.data?.message || 'Error al cancelar membresía';
            setError(errorMsg);
            throw new Error(errorMsg);
        } finally {
            setLoading(false);
    ...
    const renewMembership = async (paymentData) => {
        try {
            ...
            const errorMsg = error.response?.data?.message || 'Error al renovar membresía';
            setError(errorMsg);
            throw new Error(errorMsg);
        } finally {
            setLoading(false);
